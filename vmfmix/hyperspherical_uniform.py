...
class HypersphericalUniform(torch.distributions.Distribution):
    ...
    def __init__(self, dim, validate_args=None, device="cuda:0"):
    ...
    def __log_surface_area(self):
        if torch.__version__ >= "1.0.0":
            ...
            lgamma = torch.lgamma(torch.Tensor([(self._dim + 1) / 2])).to(self.device)
        return torch.tensor(math.log(2), device=self.device) + torch.tensor(((self._dim + 1) / 2) * math.log(math.pi), device=self.device) - lgamma
